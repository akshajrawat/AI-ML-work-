...
# ==============================================================================
# 11M RoPE + RMSNorm GPT PRETRAINING EXPERIMENT (15,000 Steps)
# ==============================================================================
# This script trains an 11M Transformer using Rotary Position Embeddings (RoPE)
# and Root Mean Square Normalization (RMSNorm) to compare against RoPE + LayerNorm.
#
# Key difference: RMSNorm removes the mean-centering step (shifting) of LayerNorm,
# normalizing only by the root mean square of inputs. This saves parameters (no bias)
# and provides faster, computationally lighter normalization with comparable stability.
# ==============================================================================
...
# Model architecture hyperparameters (~11.03M parameters with RoPE + RMSNorm)
...
# ==============================================================================
# 6. RMSNORM & TRANSFORMER ARCHITECTURE WITH RoPE
# ==============================================================================
...
class Block(nn.Module):
    """ Transformer Block: Pre-RMSNorm + Multi-Head Attention + MLP + Residual Connections """
...
class GPTLanguageModel(nn.Module):
    """ ~11M GPT using Rotary Position Embeddings (RoPE) and RMSNorm """
...
print(f"\nStarting 15,000-step 11M RoPE + RMSNorm pretraining run...")
...
for step in range(max_steps):
    ...
    if step % 500 == 0 or step == max_steps - 1:
        ...
        if losses["val"] < best_val_loss:
            ...
            torch.save(
                ...
                "checkpoints/gpt_11m_rope_rmsnorm_best.pt"
            )
            print(f"       -> [NEW BEST] Saved best RoPE+RMSNorm model (val_loss: {best_val_loss:.4f}) to 'checkpoints/gpt_11m_rope_rmsnorm_best.pt'")
    ...
    if step > 0 and step % 1000 == 0:
        torch.save(
            ...
            f"checkpoints/gpt_11m_rope_rmsnorm_step_{step}.pt"
        )
...
print("\n" + "=" * 50)
print("11M RoPE + RMSNorm TRAINING RUN COMPLETE:")
print("=" * 50)
...
torch.save(
    ...
    "checkpoints/gpt_11m_rope_rmsnorm_final.pt"
)

print("\nFinal checkpoint successfully saved to 'checkpoints/gpt_11m_rope_rmsnorm_final.pt'.")
