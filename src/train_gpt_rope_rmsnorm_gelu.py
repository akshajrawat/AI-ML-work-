...
# ==============================================================================
# 11M RoPE + RMSNorm + GELU GPT EXPERIMENT (1,000 Steps)
# ==============================================================================
# This script trains an 11M Transformer using Rotary Position Embeddings (RoPE),
# Root Mean Square Normalization (RMSNorm), and Gaussian Error Linear Units (GELU)
# in the feed-forward network, comparing against the ReLU baseline.
#
# Key difference: GELU smooths the activation threshold probabilistically based on
# the Gaussian CDF, avoiding the hard zero-gradient cutoff of ReLU for negative values.
# ==============================================================================
...
# Model architecture hyperparameters (~11.03M parameters with RoPE + RMSNorm + GELU)
...
# ==============================================================================
# 6. RMSNORM & TRANSFORMER ARCHITECTURE (RoPE + RMSNorm + GELU)
# ==============================================================================
...
class FeedForward(nn.Module):
    """ MLP / Feed-Forward Network: 4x expansion with GELU """
...
class Block(nn.Module):
    """ Transformer Block: Pre-RMSNorm + Multi-Head Attention + GELU MLP + Residual Connections """
...
class GPTLanguageModel(nn.Module):
    """ ~11M GPT using Rotary Position Embeddings (RoPE), RMSNorm, and GELU """
...
print(f"\nStarting 1,000-step 11M RoPE + RMSNorm + GELU experiment...")
...
for step in range(run_steps):
    ...
    if step % 500 == 0 or step == run_steps - 1:
        ...
        if losses["val"] < best_val_loss:
            ...
            torch.save(
                ...
                "checkpoints/gpt_11m_rope_rmsnorm_gelu_best.pt"
            )
            print(f"       -> [NEW BEST] Saved best RoPE+RMSNorm+GELU model (val_loss: {best_val_loss:.4f}) to 'checkpoints/gpt_11m_rope_rmsnorm_gelu_best.pt'")
    ...
    if step > 0 and step % 1000 == 0:
        torch.save(
            ...
            f"checkpoints/gpt_11m_rope_rmsnorm_gelu_step_{step}.pt"
        )
...
print("\n" + "=" * 50)
print("11M RoPE + RMSNorm + GELU TEST COMPLETE:")
print("=" * 50)
...
torch.save(
    ...
    "checkpoints/gpt_11m_rope_rmsnorm_gelu_final.pt"
)

print("\nFinal checkpoint successfully saved to 'checkpoints/gpt_11m_rope_rmsnorm_gelu_final.pt'.")
